<?php
/*
	Plugin Name: VidWrx Video Ads
	Plugin URI: http://vidwrx.com
	Description: Accessible & Affordable Video Ad Production. Everywhere.
	Author: VidWrx Development Team
	Author URI: http://www.vidwrx.com
	Version: 0.1
*/
/*
	Scalable Video
	Copyright 2014 SoMedia Inc.  (email : support@vidwrx.com)
...
function add_somedia_menu() {
	...
	if(get_option('somedia_username')){
		add_menu_page( 'VidWrx Video Ads', 'VidWrx Video Ads', 'edit_pages', 'VidWrx', 'somedia_page',  plugins_url('menu_icon.png', __FILE__));
		add_submenu_page( 'vidwrx', 'VidWrx Settings', 'Settings', 'edit_pages','somedia-settings', 'somedia_settings');
	} else {
		add_menu_page( 'VidWrx Video Ads', 'VidWrx Video Ads', 'edit_pages', 'somedia-settings', 'somedia_settings', plugins_url('menu_icon.png', __FILE__));
	}
...
function somedia_page() {
	...
	if(!get_option('somedia_username')){ 
		...
		<style>
			#wpbody-content { text-align: center; }
		...
		<div style="width: 772px; text-align: center; margin: 0 auto;">
			<p style="font-size:18px; font-weight: bold">Produced anywhere in North American,<br/>Delivered in 14 days.</p>
		
		</div>
		
		<form method="post" action="http://partners.vidwrx.com/login"  id="user_data">
			...
			<input type="hidden" name="domain" value="backend">
...
function somedia_shortcode( $atts ) {
	...
	if($a['id'] == ''){
		return '** VidWrx player error: no player id defined! **';
	} else {
		return '<script type="text/javascript" id="somedia_player_script_'.$a['id'].'" src="<?=$base_url->getURL("vidwrx") ?>/videoplayer_js?vid='.$a['id'].'&w='.$size.'"></script>';
	}
